...
def extract_words(text):
    ...
    words_only,word=[],''
    text=text+' '
    #appending a space at the end of 'text' to enable detection of the last word
    for char in text:
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    #words=[]
    words_list=extract_words(tweet_text(tweet))
    total_sentiment,no_of_words_with_sentiment=0,0
    for word in words_list:
        if has_sentiment(get_word_sentiment(word)):
            total_sentiment+=sentiment_value(get_word_sentiment(word))
            no_of_words_with_sentiment+=1
    if no_of_words_with_sentiment!=0:
        return make_sentiment(total_sentiment/no_of_words_with_sentiment)
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    no_of_sides=len(polygon)-1
    cx,cy,area=0,0,0
    for pos in range(no_of_sides):
        curr=polygon[pos]
        next=polygon[pos+1]
        extra=latitude(curr)*longitude(next) - latitude(next)*longitude(curr)
        #we've used extra to make things look neater  as it repeats itself in all formulae
        area=area+0.5*extra
        cx=cx+ (latitude(curr)+latitude(next))*extra
        cy=cy+ (longitude(curr)+longitude(next))*extra
    if area!=0:
       cx = cx/(6*area)
       cy = cy/(6*area)
    else:
...
def group_tweets_by_state(tweets):
    ...
    for state in us_states:
        state_centers_dict[state]=find_state_center(us_states[state])
    ...
    tweet_state_list=[]
    for tweet in tweets:
        tweet_state_list.append([find_closest_state(tweet), tweet])

    return group_by_key(tweet_state_list)
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        average_sentiment,included_tweets=0,0
        for tweet in tweet_list:
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                included_tweets+=1
                average_sentiment+=sentiment_value(analyze_tweet_sentiment(tweet))
        if included_tweets>0:
                average_sentiment/=(included_tweets)
                average_sentiment_dict[state]=average_sentiment
